...
The `generatedAt` field is the version timestamp of the frozen Phase 7A evidence snapshot, not a wall-clock build timestamp. It changes only when the formal evidence snapshot is intentionally rebaselined, so a clean checkout can reproduce the same manifest without depending on a prior ignored output file.
...
Binary reproducibility is enforced after content generation. Selected MATLAB PNGs are re-encoded atomically to preserve pixels while removing volatile creation metadata, report text assets use repository-pinned LF checkout endings, and DOCX ZIP entries receive fixed metadata and deterministic ordering. The Word-produced PDF is then normalized to remove volatile metadata and assign fixed document dates and identifiers before its hash is recorded. The normalizers do not alter report text, figure pixels, tables, or pagination.
...
- **Volatile build metadata:** version the frozen evidence snapshot explicitly, strip volatile PNG metadata without changing pixels, pin report text checkout endings, normalize DOCX ZIP metadata and ordering, and normalize Word PDF metadata and identifiers before hashing.
